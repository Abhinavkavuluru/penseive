...
class ActorNetwork(object):
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor'):
            ...
            # Reshape for dense layers (take last timestep)
            split_0 = tf.reshape(inputs[:, 0:1, -1], [-1, 1])
            split_0 = tf.layers.dense(split_0, 128, activation=tf.nn.relu)
            
            split_1 = tf.reshape(inputs[:, 1:2, -1], [-1, 1])
            split_1 = tf.layers.dense(split_1, 128, activation=tf.nn.relu)
            
            # Reshape for conv1d (needs 3D: batch, time, features)
            split_2 = tf.reshape(inputs[:, 2:3, :], [-1, self.s_dim[1], 1])
            split_2 = tf.layers.conv1d(split_2, 128, 4, activation=tf.nn.relu)
            
            split_3 = tf.reshape(inputs[:, 3:4, :], [-1, self.s_dim[1], 1])
            split_3 = tf.layers.conv1d(split_3, 128, 4, activation=tf.nn.relu)
            
            split_4 = tf.reshape(inputs[:, 4:5, :A_DIM], [-1, A_DIM, 1])
            split_4 = tf.layers.conv1d(split_4, 128, 4, activation=tf.nn.relu)
            
            split_5 = tf.reshape(inputs[:, 4:5, -1], [-1, 1])
            split_5 = tf.layers.dense(split_5, 128, activation=tf.nn.relu)

            split_2_flat = tf.layers.flatten(split_2)
            split_3_flat = tf.layers.flatten(split_3)
            split_4_flat = tf.layers.flatten(split_4)
            ...
            out = tf.layers.dense(dense_net_0, self.a_dim, activation=tf.nn.softmax)
